...
output_dir = "results/mongo_index"
...
def setup_index():
    ...
    db.messages.create_index([("userId", 1), ("timestamp", 1)])
...
if __name__ == "__main__":
    ...
    # ===== SEKVENTIELLT TEST =====
    with open(f"{output_dir}/single_index.csv", "w", newline="") as f:
        writer = csv.writer(f)
    ...
    # ===== PARALLELLT TEST =====
    with open(f"{output_dir}/multi_index.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["size", "run", "insert", "query"])
